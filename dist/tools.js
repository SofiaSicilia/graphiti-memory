 ...
 */
export async function initializeGraphiti(config) {
    if (memoryInstance)
        return;
    const fullConfig = {
 ...
 */
export async function graphiti_recall(query, options) {
    if (!memoryInstance) {
    ...
    const parts = ['## Graphiti Memory Results:\n'];
 ...
 */
export async function graphiti_add_fact(fact, metadata) {
    if (!memoryInstance) {
        throw new Error('Graphiti Memory not initialized. Call initializeGraphiti() first.');
    }
 ...
 */
export async function graphiti_get_context(message) {
    if (!memoryInstance) {
        return '';
    }
 ...
 */
export async function graphiti_capture(userMessage, assistantResponse, context) {
    if (!memoryInstance)
        return;
    await memoryInstance.captureConversation(userMessage, assistantResponse, context?.channelId, context?.threadId);
}
/**
 ...
 */
export async function closeGraphiti() {
    if (memoryInstance) {
