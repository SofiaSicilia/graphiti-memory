...
export class GraphitiClient {
    ...
    async detectVectorCapabilities() {
        if (!this.client)
        ...
        try {
            ...
            const extResult = await this.client.query("SELECT 1 FROM pg_extension WHERE extname = 'vector'");
    ...
    parseEmbedding(row) {
        if (row.embedding_vector) {
            ...
            if (typeof vec === 'string') {
                ...
                return vec.replace(/[\[\]]/g, '').split(',').map((x) => parseFloat(x.trim()));
            }
        }
        if (row.embedding) {
            // JSONB format
     ...
     */
    async semanticSearchEntities(embedding, limit = 5, threshold = 0.7) {
        if (!this.client)
            throw new Error('Client not initialized');
        const hasVector = this.vectorCapabilities?.hasVectorExtension ?? false;
        const hasVectorCols = this.vectorCapabilities?.hasVectorColumns ?? false;
        ...
        try {
            ...
            const result = await this.client.query(`SELECT *, embedding_vector <=> $1 as distance 
     ...
     */
    async semanticSearchEpisodes(embedding, limit = 5, threshold) {
        if (!this.client)
            throw new Error('Client not initialized');
        const hasVector = this.vectorCapabilities?.hasVectorExtension ?? false;
        const hasVectorCols = this.vectorCapabilities?.hasVectorColumns ?? false;
     ...
     */
    async hybridSearchEpisodes(queryText, queryEmbedding, limit = 5, semanticWeight = 0.7) {
        if (!this.client)
            throw new Error('Client not initialized');
        const hasVector = this.vectorCapabilities?.hasVectorExtension ?? false;
        const hasVectorCols = this.vectorCapabilities?.hasVectorColumns ?? false;
        ...
        try {
            ...
            const result = await this.client.query(`WITH semantic_scores AS (
          SELECT 
            id,
        ...
        ORDER BY score DESC
        LIMIT $2`, [vectorStr, limit, `%${queryText}%`, `%${queryText.split(' ').join('%')}%`, semanticWeight, textWeight]);
